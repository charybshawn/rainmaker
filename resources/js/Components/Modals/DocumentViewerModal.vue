...
const emit = defineEmits(['close', 'edit', 'delete', 'update'])
...
const startEditing = (field) => {
  ...
  if (field === 'category') {
    editValues.value.category = props.document?.category?.id || null
  } else if (field === 'visibility') {
    editValues.value.visibility = props.document?.visibility || 'private'
  }
}

const cancelEditing = (field) => {
  isEditing.value[field] = false
  editValues.value[field] = field === 'category' ? null : ''
}
