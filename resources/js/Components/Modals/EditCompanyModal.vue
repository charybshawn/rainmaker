            ...
            <form class="space-y-6">
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="space-y-6">
                  ...
                  <!-- Headquarters -->
                  <div>
                    <label for="edit_headquarters" class="block text-sm font-medium text-white mb-2">Headquarters</label>
                    <input
                      id="edit_headquarters"
                      :value="editForm.headquarters"
                      @input="$emit('update:edit-form', { ...editForm, headquarters: $event.target.value })"
                      type="text"
                      class="w-full px-4 py-3 rounded-xl bg-black/10 backdrop-blur-xl border border-white/20 text-white placeholder-gray-400 shadow-[0_4px_12px_0_rgba(31,38,135,0.15)] focus:shadow-[0_4px_16px_0_rgba(59,130,246,0.2)] focus:border-blue-400/50 focus:ring-2 focus:ring-blue-400/20 transition-all duration-500"
                      style="backdrop-filter: blur(20px) saturate(180%);"
                      placeholder="e.g., Cupertino, California"
                    />
                    <div v-if="editErrors.headquarters" class="text-red-400 text-sm mt-1">{{ editErrors.headquarters }}</div>
                  </div>

                  <!-- Employees -->
                  <div>
                    <label for="edit_employees" class="block text-sm font-medium text-white mb-2">Number of Employees</label>
                    <input
                      id="edit_employees"
                      :value="editForm.employees"
                      @input="$emit('update:edit-form', { ...editForm, employees: parseInt($event.target.value) || null })"
                      type="number"
                      min="1"
                      class="w-full px-4 py-3 rounded-xl bg-black/10 backdrop-blur-xl border border-white/20 text-white placeholder-gray-400 shadow-[0_4px_12px_0_rgba(31,38,135,0.15)] focus:shadow-[0_4px_16px_0_rgba(59,130,246,0.2)] focus:border-blue-400/50 focus:ring-2 focus:ring-blue-400/20 transition-all duration-500"
                      style="backdrop-filter: blur(20px) saturate(180%);"
                      placeholder="e.g., 147000"
                    />
                    <div v-if="editErrors.employees" class="text-red-400 text-sm mt-1">{{ editErrors.employees }}</div>
                  </div>

                  <!-- Founded Date -->
                  <div>
                    <label for="edit_founded_date" class="block text-sm font-medium text-white mb-2">Founded Date</label>
                    <input
                      id="edit_founded_date"
                      :value="editForm.founded_date"
                      @input="$emit('update:edit-form', { ...editForm, founded_date: $event.target.value })"
                      type="date"
                      class="w-full px-4 py-3 rounded-xl bg-black/10 backdrop-blur-xl border border-white/20 text-white shadow-[0_4px_12px_0_rgba(31,38,135,0.15)] focus:shadow-[0_4px_16px_0_rgba(59,130,246,0.2)] focus:border-blue-400/50 focus:ring-2 focus:ring-blue-400/20 transition-all duration-500"
                      style="backdrop-filter: blur(20px) saturate(180%);"
                    />
                    <div v-if="editErrors.founded_date" class="text-red-400 text-sm mt-1">{{ editErrors.founded_date }}</div>
                  </div>
                </div>
              </div>
              ...
              <div class="space-y-6">
                <!-- Website -->
                <div>
                  <label for="edit_website" class="block text-sm font-medium text-white mb-2">Website</label>
                  <input
                    id="edit_website"
                    :value="editForm.website"
                    @input="$emit('update:edit-form', { ...editForm, website: $event.target.value })"
                    type="url"
                    class="w-full px-4 py-3 rounded-xl bg-black/10 backdrop-blur-xl border border-white/20 text-white placeholder-gray-400 shadow-[0_4px_12px_0_rgba(31,38,135,0.15)] focus:shadow-[0_4px_16px_0_rgba(59,130,246,0.2)] focus:border-blue-400/50 focus:ring-2 focus:ring-blue-400/20 transition-all duration-500"
                    style="backdrop-filter: blur(20px) saturate(180%);"
                    placeholder="https://www.company.com"
                  />
                  <div v-if="editErrors.website" class="text-red-400 text-sm mt-1">{{ editErrors.website }}</div>
                </div>
